...
MODE = "prompt"
DEFAULT_LANG = LANGUAGE["MA"]   # Default language in which subtitles
                                # are downloaded.
...
def zip_extractor(name):
    ...
    try:
        with zipfile.ZipFile(name, "r") as z:
            ...
            if len(zfile_list)>1:
                for index in range(zfile_list):
                    print("({}): {}".format(index,zfile_list[index]))
                file_ind = input("Which one?:")
                z.extract(zfile_list[file_ind])
            else:
                z.extractall()
                pass
        os.remove(name)
...
def sel_sub(page, name="", lang=DEFAULT_LANG):
    ...
    for link in soup.find_all('td', {'class': 'a1'}):
        ...
        if 'trailer' not in link.text.lower()\
            ...
            link_text = link.find_all('span')[1].text.strip().encode('cp850','replace')
            if name.lower() in link.text.lower():
                sub_list.append({"url":host+link.get('href'),"name":link_text})
                current_sub += 1
